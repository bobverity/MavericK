...
void run_TI_MCMC_noAdmixture(globals &globals, int Kindex) {
    ...
    // perform MCMC
    MCMC_TI_noAdmixture TI_MCMC(globals, Kindex, globals.thermodynamicRungs);
    TI_MCMC.perform_MCMC();
    
    // save results to global variable
    globals.TIpoint_mean[Kindex] = TI_MCMC.TIpoint_mean;
    globals.TIpoint_var[Kindex] = TI_MCMC.TIpoint_var;
    globals.TIpoint_SE[Kindex] = TI_MCMC.TIpoint_SE;
    globals.logEvidence_TI[Kindex] = TI_MCMC.logEvidence_TI;
    globals.logEvidence_TI_SE[Kindex] = TI_MCMC.logEvidence_TI_SE;
